(function () {
  ...
  var QBIT_PROFILE_IDS$1 = {
    home: true,
    dacha: true,
    pc: true
  };
  var DEFAULT_QBIT_PROFILE$1 = "home";
  var DEFAULT_QBIT_PROFILE_TITLES$1 = {
    home: "Дом",
    dacha: "Дача",
    pc: "ПК"
  };
  function normalizeProfileId$1(profileId) {
    return QBIT_PROFILE_IDS$1[profileId] ? profileId : DEFAULT_QBIT_PROFILE$1;
  }
  function profileFieldKey(profileId, suffix) {
    var normalizedProfileId = normalizeProfileId$1(profileId);
    var profileName = normalizedProfileId.charAt(0).toUpperCase() + normalizedProfileId.slice(1);
    return "".concat(CONFIG_PREFIX$1).concat(profileName).concat(suffix);
  }
  function getProfileTitle$1(profileId) {
    var normalizedProfileId = normalizeProfileId$1(profileId);
    var customTitle = sanitizeConfigString(Lampa.Storage.field(profileFieldKey(normalizedProfileId, "Title")));
    return customTitle || DEFAULT_QBIT_PROFILE_TITLES$1[normalizedProfileId] || DEFAULT_QBIT_PROFILE_TITLES$1[DEFAULT_QBIT_PROFILE$1];
  }
  function getConfig$1(profileId) {
    var normalizedProfileId = normalizeProfileId$1(profileId);
    return {
      url: sanitizeConfigString(Lampa.Storage.field(profileFieldKey(normalizedProfileId, "Url"))),
      user: sanitizeConfigString(Lampa.Storage.field(profileFieldKey(normalizedProfileId, "User"))),
      pass: sanitizeConfigString(Lampa.Storage.field(profileFieldKey(normalizedProfileId, "Pass"))),
  ...
  function getHeaders$1(profileId, contentType) {
    var config = getConfig$1(profileId);
    var headers = {};
    if (contentType) {
  ...
  function ajaxRequest(profileId, path) {
    var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        ...
        while (1) switch (_context.n) {
          case 0:
            if (typeof path === "undefined") {
              path = profileId;
              profileId = DEFAULT_QBIT_PROFILE$1;
            }
  ...
  function SendTask$1(_x4, _x5, _x6, _x7) {
    return _SendTask$1.apply(this, arguments);
  }
  function _SendTask$1() {
    _SendTask$1 = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee7(profileId, selectedTorrent, labels, dtype) {
      var _t10;
      return _regenerator().w(function (_context7) {
        while (1) switch (_context7.n) {
          ...
          case 1:
            _context7.p = 1;
            _context7.n = 2;
            return auth$1(profileId, false);
          case 2:
            _context7.n = 3;
            return makeRequest(profileId, "/api/v2/torrents/add", {
              method: "POST",
              contentType: "application/x-www-form-urlencoded; charset=UTF-8",
              data: {
                tags: labels || "",
  ...
  function createRpcTorrentClient() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var configPrefix = options.configPrefix;
    var clientName = options.clientName || "RPC torrent client";
    ...
    if (!configPrefix) {
      throw new Error("configPrefix is required for rpcTorrentClient");
    }
    function getConfig() {
  ...
  var Keenetic = createRpcTorrentClient({
    ...
    _updateMetadataForTorrent = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee(torrent, client) {
      ...
      return _regenerator().w(function (_context) {
        while (1) switch (_context.n) {
          ...
          case 2:
            _t = client.type;
            _context.n = _t === 'keenetic' ? 3 : _t === 'qbittorrent' ? 5 : _t === 'synology' ? 7 : 9;
            break;
          case 3:
            _context.n = 4;
            return updateKeeneticMetadata(torrent, client, mediaInfo);
          case 4:
  ...
  /**
   * Оновлення метаданих для Keenetic
   *
   * @param {Object} torrent - об'єкт торента
   * @param {Object} client - інформація про клієнта
   * @param {Object} mediaInfo - інформація про медіа з TMDB
   */
  function updateKeeneticMetadata(_x5, _x6, _x7) {
    return _updateKeeneticMetadata.apply(this, arguments);
  }
  /**
   ...
   * @param {Object} mediaInfo - інформація про медіа з TMDB
   */
  function _updateKeeneticMetadata() {
    _updateKeeneticMetadata = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee4(torrent, client, mediaInfo) {
      var label, _t3;
      return _regenerator().w(function (_context4) {
        while (1) switch (_context4.n) {
          case 0:
          ...
          case 3:
            _context4.p = 3;
            _t3 = _context4.v;
            console.error('TDM', 'updateKeeneticMetadata error:', _t3);
            Lampa.Bell.push({
  ...
  function Main$1(manifest) {
    ...
    function updateSecretLabel(item, label, storageKey) {
      var status = hasSecretValue(Lampa.Storage.field(storageKey)) ? "set" : "empty";
    ...
    function createAddressValidationHandler(storageKey) {
      return function (newValue) {
        ...
        if (!isAddressWithProtocol(normalizedValue)) {
          Lampa.Storage.set(storageKey, normalizeInputValue(Lampa.Storage.field(storageKey)));
          Lampa.Bell.push({
    ...
    function addQbitProfileSettings(profileId, title) {
      var profileName = profileId.charAt(0).toUpperCase() + profileId.slice(1);
      var baseKey = manifest.component + "qBittorent" + profileName;
      Lampa.SettingsApi.addParam({
        ...
        onChange: function onChange(value) {
          Lampa.Storage.set(baseKey + "Title", normalizeInputValue(value));
          Lampa.Settings.update();
        }
      });
      Lampa.SettingsApi.addParam({
        ...
        onChange: createAddressValidationHandler(baseKey + "Url")
      });
      Lampa.SettingsApi.addParam({
        component: manifest.component,
        param: {
        ...
        onChange: function onChange(value) {
          Lampa.Storage.set(baseKey + "User", normalizeInputValue(value));
          Lampa.Settings.update();
        }
      });
      Lampa.SettingsApi.addParam({
        ...
        onRender: function onRender(item) {
          updateSecretLabel(item, title + " пароль", baseKey + "Pass");
          item.show();
        },
        onChange: function onChange() {
          openSecretEditor(baseKey + "Pass", title + " пароль");
        }
      });
    }

    addQbitProfileSettings("home", "Дом");
    addQbitProfileSettings("dacha", "Дача");
    addQbitProfileSettings("pc", "ПК");
  }
  ...
  var QBIT_PROFILE_MENU = ["home", "dacha", "pc"];
  function sendToQbitProfile(profileId, selectedTorrent, labels, dtype) {
    executeClientMethod("qBittorent", "SendTask", [profileId, selectedTorrent, labels, dtype], {
      silentAuth: true
    })["catch"](function () {});
  }
      ...
      e.menu.push({
        ...
        onSelect: function onSelect() {
          Lampa.Select.show({
            ...
            items: QBIT_PROFILE_MENU.map(function (profileId) {
              return {
                title: getProfileTitle$1(profileId),
                profileId: profileId
              };
            }),
            onSelect: function onSelect(profileItem) {
              if (!profileItem || !profileItem.profileId) {
                return;
              }
              sendToQbitProfile(profileItem.profileId, selectedTorrent, labels, dtype);
            }
          });
        }
      });
  ...
  function initializeClientAuth() {
    Qbittorent.auth("home", false)["catch"](function () {});
  }
